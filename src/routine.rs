    ...
    T: ?Sized,
{
    fn bench(&mut self, m: &M, iters: &[u64], parameter: &T) -> Vec<f64> {
        ...
        for (i, iters) in iters.iter().enumerate() {
            #[cfg(all(feature = "stack_randomization", any(target_family = "unix", target_family = "windows")))]
            {
            ...
            #[cfg(not(all(feature = "stack_randomization", any(target_family = "unix", target_family = "windows"))))]
            {
                b.iters = *iters;
                (*f)(&mut b, black_box(parameter));
                b.assert_iterated();
